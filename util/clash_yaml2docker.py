...
def generateDockerComm(yaml_file_path:str, dockerName:str, port=7890, ui_port=9090):
    ...
    shell_params = [
        ...
        f"--restart always",
        f"-p {port}:7890",
        f"-p {port+100}:7891/udp",
        f"-p {f'{ui_port}'}:9090",
        f"-v {yaml_file_path}:/root/.config/clash/config.yaml",
        # f"-v {yaml_file_path}:/etc/clash/config.yml",
        # f"dreamacro/clash"        # 开源版
        f"dreamacro/clash-premium" # 闭源版
        # f"-v /path/ui:/ui",   # 用不上
    ]
    comm = ""
...
def resultDocker_comm(new_comm, old_comm, dockered):
    """
    整合处理各个容器需要运行的命令
    有变化的docker容器重启，
    没出现过的docker批量运行
    ...
    :param dockered: docker 已启动名单和容器编号映射关系
    :return: 整合后的需要运行的命令
    """
    result_comms = {}
    if old_comm:
        ...
        for k, v in old_comm.items():                           # {'clash—7890-xx': 'docker restart a4e0c2bc78e4'}
            for m, n in new_comm.items():                       # {'clash—7910-xx': "docker run -d --name 'clash-7910-1649399371900' --restart always -p 7910:7890 -p 7911:7891 -p 9100:9090 -v /home/ubuntu/software/clash/configs/1649399371900.yml:/root/.config/clash/config.yaml dreamacro/clash"}
                k_temp = k.split("-")[1]    # 获取旧docker所有占用的端口号
                m_temp = m.split("-")[1]    # 获取新docker将要占用的端口号
                if k_temp == m_temp:    # 端口占用判断
                    if k == m:  # 容器名称完全相同
                        ...
                        result_comms[k] = [
                            f"docker stop {docker_id}", # 关闭旧容器
                            f"docker container rm {docker_id}",  # 删除容器实例
                            n,  # docker run 一个新容器
                        ]
                else:
                    print(f'{m} 为全新容器!')
...
if __name__ == '__main__':
    ...
    # 整合命令
    docker_comms_result = resultDocker_comm(docker_comms,dockered_comms,dockered)
    ...
    for a,b in docker_comms_result.items():
        ...
        temp_consoles = []
        for c in b:
            temp_consoles.append(cmdRuner(c))
            time.sleep(3)
        docker_runs[a] = temp_consoles
